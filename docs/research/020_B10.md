...
The first twenty-nine bounded pulses now read like this:
...
| pulse source-order start | thematic split from `20082..20096` |
| pulse size | `3` |
...
- twenty-sixth `Beta 1.0` pulse rollover:
...
- twenty-seventh `Beta 1.0` pulse rollover:
...
- twenty-eighth `Beta 1.0` pulse rollover:
...
- current live `Beta 1.0` pulse:
  - `active neutral proof surface 20103..20105`
  - `3` live rows
  - `.local/evals.sqlite`

Archive labels preserve the quarantine annotations. Current chart labels and
lane reads derive family from row data when those surfaces disagree.
...
The twenty-seventh pulse tests the lilac / mauve split after the undertone
bucket correction:
...
The twenty-eighth pulse tests the blue / jade split after the same correction:

- the split comes back at `3 anchors / 0 counted seams`
- `Sheer pink`, `Ecru`, and `Navajo` no longer jump into blue or jade
  replacements

The twenty-ninth pulse tests the mint / green split after the same correction:

- the split comes back at `3 anchors / 0 counted seams`
- `Dew`, `Powder puff`, and `Bridal blush` no longer jump into mint or green
  replacements
- the live question moves from targeted correction to whether a broader
  `neutral` continuation from source order `48` is useful
...
- whether the corrected `neutral` lane should take a broader continuation from
  source order `48`
...
4. decide whether another bounded `neutral` continuation from source order
   `48` is useful after the split correction
