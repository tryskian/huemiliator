...
- brown boundary refinement:
  ...
  - residual muted olive and khaki shoulders now leave `brown` before the dark
    earthy shortcut
  - residual loud orange, yellow, and gold shoulders now leave `brown` before
    ranking
...
5. The runtime classifies the matched swatch into a closed Huemiliator-owned
   ...
   - bright gold and ochre shoulder colours can fall through to `orange` or
     `yellow`
   - muted olive, khaki, and loud orange/gold/yellow shoulders are excluded
     from the active `brown` lane
